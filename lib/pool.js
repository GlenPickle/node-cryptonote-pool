...
var bannedIPs = {};
var perIPStats = {};
var walletsPerIP = [{ ip: 0, wallets: 0 }];
...
function newWallet(wallet, wallet_array){
    for (var i = 0; i < wallet_array.length; i++){
        if (wallet_array[i] == wallet){
            return i;
        }
    }
    return -1;
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            // Check if this IP has logged in before
            var ip_index = newIP(miner.ip, walletsPerIP);
            if (ip_index == -1){
                walletsPerIP.push({
                    ip: miner.ip,
                    wallets: [params.login]
                });
                ip_index = walletsPerIP.length - 1;
            }
            else {
                var wallet_index = newWallet(params.login, walletsPerIP[ip_index].wallets);
                if (wallet_index == -1){
                    walletsPerIP[ip_index].wallets.push(params.login);
                }
            }
            // If too many wallets from the same IP, ban it
            if (walletsPerIP[ip_index].wallets.length > 5){
                log('warn', logSystem, 'Banned %s@%s', [params.login, miner.ip]);
                bannedIPs[miner.ip] = Date.now();
                delete connectedMiners[miner.id];
                process.send({type: 'banIP', ip: miner.ip});
            }
