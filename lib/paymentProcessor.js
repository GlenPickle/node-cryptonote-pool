...
function runInterval(){
    async.waterfall([
        ...
        function(workers, callback){
            ...
            /* TODO find the block reward */
            var block_reward = 5.1;
            num_payouts = Math.floor(block_reward);
            log('info', logSystem, 'workers is %j', [workers]);
            for (var j = 0; j < num_payouts; j++){
               log('info', logSystem, 'finding random integer from 0 to %j', [workers.length - 1]);
               payments[j] = Random.integer(0,workers.length - 1);
               log('info', logSystem, 'miner number %j has been chosen', [workers[payments[j]]]);
            }
